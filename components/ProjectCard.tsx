...
const Heading = ({
  ...
  return (
    <div className={cn('max-w-[50rem] mx-auto mb-12 lg:mb-20 md:text-center', className)}>
...
const ProjectCard = ({
  ...
  return (
    <div
      ...
      // style={{ backgroundImage: 'url(/assets/borderGradient/card-2.svg)' }}
    >
      <div className='pointer-events-none relative z-2 flex min-h-[22rem] flex-col p-[2.4rem]'>
        ...
        <div className='  relative flex max-h-20 w-60 flex-col items-center'>
          <div className='p-4'>
            {image && <Image src={image} alt={'item.title'} width={300} height={200} className=' ' />}
          </div>
          <p className=' ml-auto text-xs font-bold uppercase tracking-wider text-n-1'>Explore more</p>
          <Arrow />
        </div>
      </div>
      <GradientLight />
      <div className='absolute inset-0.5 bg-n-8' style={{ clipPath: 'url(#benefits)' }}>
        <div className='absolute inset-0 opacity-0 transition-opacity hover:opacity-10'>
          {image && (
            <Image src={image} alt={'item.title'} width={380} height={362} className='size-full object-cover' />
          )}
        </div>
      </div>
      <ClipPath />
